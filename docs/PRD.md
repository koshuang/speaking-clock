...
#### 2.1.10 遊戲化功能 - 星星獎勵系統（v1.10.0 新增）
| 需求 | 描述 | 優先級 |
|------|------|--------|
| 任務完成獎勵 | 完成任務獲得 1 顆星星 | P1 ✅ |
| 準時完成獎勵 | 在時間內完成額外獲得 1 顆星星 | P1 ✅ |
| 連擊獎勵 | 連續完成 3 個任務額外獲得 2 顆星星 | P1 ✅ |
| 每日完成獎勵 | 當天任務全部完成額外獲得 3 顆星星 | P1 ✅ |
| 星星計數器 | Header 顯示今日累積星星數 | P1 ✅ |
| 每日進度環 | 圓形進度環顯示任務完成度與星星進度 | P1 ✅ |
| 星星動畫 | 獲得星星時顯示飛入動畫與數字跳動 | P1 ✅ |
...
| 終極目標同步 | 登入後自動同步終極目標到雲端 | P1 ✅ |
| 離線優先 | localStorage 優先，背景同步到雲端 | P1 ✅ |
| 即時同步 | 多裝置間即時同步資料變更（Supabase Realtime） | P2 ✅ |
| 使用者選單 | 顯示使用者頭像與名稱，提供登出功能 | P1 ✅ |

#### 2.1.12 終極目標功能（v1.13.0 新增）
| 需求 | 描述 | 優先級 |
|------|------|--------|
| 目標設定 | 設定終極目標名稱與目標時間（如 07:50 出門） | P1 ✅ |
| 多組目標 | 支援建立多組終極目標 | P1 ✅ |
| 待辦關聯 | 可將待辦事項關聯到特定目標 | P2 ✅ |
| 倒數顯示 | 首頁顯示目標倒數時間與緊急程度 | P1 ✅ |
| 緊急程度 | 依剩餘時間變色（綠→黃→橘→紅） | P1 ✅ |
| 語音提醒 | 報時時播報距離目標還有多少分鐘 | P1 ✅ |
| 超時警告 | 超過目標時間後持續提醒 | P2 ✅ |
| 雲端同步 | 登入後自動同步終極目標到雲端 | P1 ✅ |
...
| 資料庫 Migration | Supabase CLI | 版本化資料庫變更管理 |
| 測試 | Vitest | 單元測試 |
